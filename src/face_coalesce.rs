...
fn coalesce_inner(entities: &mut Vec<EntityInstance>) -> Option<FaceCoalesceStats> {
    ...
    for users in edge_faces.values() {
        ...
        let (Some(ai), Some(bi)) = (face_info.get(&a), face_info.get(&b)) else {
            continue;
        };
        if ai.support != bi.support || ai.sense != bi.sense {
            continue;
        }
    ...
    for root in roots {
        ...
        let Some(first_owners) = face_shells.get(&canonical) else {
            continue;
        };
        if first_owners.len() != 1 {
            continue;
        }
        if component
            .iter()
            .any(|f| face_shells.get(f) != Some(first_owners))
        {
            continue;
        }
        ...
        for &face in &component {
            ...
            if face != canonical
                && let Some(v) = styles.get(&face)
            {
                for (style, _) in v {
                    style_delete.insert(*style);
                }
            }
        }
        if !style_ok {
        ...
        if !ownership_ok || internal_edges.is_empty() {
            continue;
        }

        let mut surviving = Vec::<(u64, u64)>::new();
        for face in &component {
            let Some(info) = face_info.get(face) else {
                ownership_ok = false;
                break;
            };
        ...
        if !ownership_ok || surviving.is_empty() {
            continue;
        }
        ...
        if edge_counts.values().any(|&n| n != 1) {
            continue;
        }
        ...
        let Some(loop_oes) = trace_single_loop(&surviving, entities, &index) else {
            continue;
        };
...
fn parse_face_info(
    ...
    for bound in bounds {
        ...
        if !logical_bool(bound_params.get(2)?)? {
            return None;
        }
...
fn face_shell_owners(
    ...
    for entity in entities {
        ...
        if !matches!(record.name.as_str(), "OPEN_SHELL" | "CLOSED_SHELL") {
            continue;
        }
        let Parameter::List(params) = &record.parameter else {
            continue;
        };
        let Some(faces) = params.get(1).and_then(entity_ref_list) else {
            continue;
        };
        for face in faces {
            if index
...
fn rewrite_shell_faces(shell: &mut EntityInstance, mapping: &HashMap<u64, u64>) -> Option<()> {
    ...
    let Parameter::List(faces) = params.get_mut(1)? else {
        return None;
    };
